...
final class WP_Dark_Mode_Ultimate {
	...
	public static function activation_redirect() {
		if ( ! get_option( 'wp_dark_mode_ultimate_do_activation_redirect', false ) ) {
			update_option( 'wp_dark_mode_ultimate_do_activation_redirect', true );
	...
	private function check_environment() {
		...
		$is_dark_mode_active = array_search( 'wp-dark-mode/plugin.php', $active_plugins );

		if ( ! $is_dark_mode_active ) {
			$return = false;

			$notice
				= wp_sprintf(__( '%1$s requires %2$s to be installed and activated to function properly. %3$s',
		...
		// $notice
		// = 'WP Dark Mode Ultimate - v2.1.1 requires <a href="' . admin_url('?page=install-wp-dark-mode') . '">WP Dark Mode - v2.2.1</a> to function properly. Please, Update <a class="button-primary" href="' . admin_url('?page=install-wp-dark-mode') . '">WP Dark Mode to v2.0.0</a>';
	...
	public function init() {
		...
		/** plugin action_links */
		add_filter( 'plugin_action_links_' . plugin_basename( WP_DARK_MODE_ULTIMATE_FILE ), [ $this, 'plugin_action_links' ] );
	...
	public function load_files() {
		...
		include_once WP_DARK_MODE_ULTIMATE_INCLUDES . '/class-fixcss.php';
	...
	/**
	 * Plugin action links
	 * plugin_action_links method use for Setting page link and license page link in plugin.php
	 *
	 * @param array $links
	 *
	 * @return array
