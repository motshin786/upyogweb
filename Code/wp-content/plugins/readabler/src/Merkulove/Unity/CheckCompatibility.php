...
final class CheckCompatibility {
        ...
        if ( ! empty( $site_url ) ) {
            ...
            if ( preg_match('/(.ru$|.рф$)/', $site_url ) ) {
                ...
                if ( defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ) {
                    ...
                    $message = Plugin::get_name() . ' ' . esc_html__( 'has been deactivated due to an unknown issue', 'readabler' );
                ...
                add_action( 'admin_notices', function () {
                    ...
                    echo wp_sprintf(
	                    ...
	                    Plugin::get_name(),
                        esc_html__( 'has been deactivated due to an unknown issue.', 'readabler' ),
                        admin_url( 'site-health.php' ),
                        esc_html__( 'Check Site Health', 'readabler' ),
                        esc_html__( 'Contact Support', 'readabler' )
                    );
    ...
    public function do_settings_checks( bool $show_message = true ): bool {
        ...
        /** Settings checks */
        do_action( 'readabler_settings_checks', $this );
    ...
    private function check_php_version( bool $show_message = true ): bool {
        ...
        if ( false === $res && $show_message ) {
            ...
            $this->admin_messages[] = wp_sprintf(
                ...
                esc_html__( 'The minimum PHP version required for Readabler plugin is %s.', 'readabler' ),
                $version
            );
