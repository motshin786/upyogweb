...
final class PluginUpdater {
     ...
     * @var string
	 **/
	public $opt_plugin_info = 'mdp_readabler_plugin_info';
	...
	private function __construct() {
		...
		add_action( 'in_plugin_update_message-readabler/readabler.php', [$this, 'show_plugin_update_message' ], 10, 2 );
	...
	private function compare_with_new( $plugin_info, $wp_version, $update_plugins ) {
		...
		if ( version_compare( $latest_version, $current_version ) > 0 ) {
			...
			$update_plugins->response['readabler/readabler.php'] = (object)[
				'slug' => 'readabler',
                'plugin' => 'readabler/readabler.php',
	...
	public function plugin_info( $res, $action, $args ) {
		...
		if ( 'readabler' !== $args->slug ) { return false; }
	...
	private function get_plugin_information() {
        ...
        if ( ! is_array( $plugin_info ) ) {
	        ...
	        if ( ! $plugin_info->mdp_readabler_plugin_info ) {
		        return false;
	        } else {
		        $plugin_info = json_decode( $plugin_info->mdp_readabler_plugin_info->body, false );
	        }
        }
	...
	public function show_plugin_update_message( $plugin_data, $r ) {
		...
		if ( ! TabActivation::get_instance()->is_activated() ) {
            ...
            esc_html_e( 'Your plugin license is ', 'readabler' );
            echo '<strong style="color: #ff495c">';
            esc_html_e( 'not activated', 'readabler' );
            echo '.</strong> ';
            esc_html_e( 'Please activate the license to receive automatic updates in one click, or visit the plugin page on the Envato market to ', 'readabler' );
			...
			esc_html_e( 'download ', 'readabler' );
			echo '</a> ';
			esc_html_e( 'the latest version.', 'readabler' );
		}
