...
final class UI {
    ...
    public function render_icon( $value, string $label = '', string $helper_text = '', array $attributes = [], array $meta = [] ) {
        ...
        <div <?php echo ( $class ) ? 'class="' . esc_attr( $class ) . '"' : ' '; ?>>
            ...
            <div class="mdc-icon-field-img-box <?php if ( $src ) : ?>mdc-with-image<?php endif; ?>">
                ...
                ><?php esc_html_e( 'Icon Library', 'readabler' ); ?></button>
                <button class="mdc-icon-field-upload-btn mdc-button mdc-button--outlined mdc-ripple-upgraded"><?php esc_html_e( 'Upload SVG', 'readabler' ); ?></button>
            </div>
	...
	public function render_media_library( $value, string $helper_text = '', array $attributes = [] ) {
		...
		$image = intval( $value ) > 0 ?
			...
			wp_sprintf(
				...
				$id . '-preview-image',
				esc_html__( 'Preview image' , 'readabler' )
			);
		...
		echo wp_sprintf(
			...
			$image,
			esc_attr__( 'Remove', 'readabler' ),
			esc_attr( $id ),
			esc_html__( 'Media library', 'readabler' )
		);
    ...
    public function render_file_dnd( $key, $tab, $field ) {
        ...
        <div class="mdp-dnd mdp-file">
            ...
            echo wp_sprintf(
                ...
                isset( $field[ 'label' ] ) ? esc_html( $field[ 'label' ] ) : esc_html__( 'File', 'readabler' )
            );
            ...
            if ( $file_exist ) :
                ...
                echo wp_sprintf(
                    ...
                    esc_html__( 'File exist', 'readabler' ),
                    esc_html__( 'Drop file here or click to upload', 'readabler' )
                );
            ...
            if ( $file_exist ) :
                ...
                echo wp_sprintf(
                    ...
                    esc_html__( 'Drag and drop or click on the form to replace file.', 'readabler' ),
                    esc_html__( 'Reset', 'readabler' ),
	                $field[ 'description' ] ?? ''
                );
                ...
                echo wp_sprintf(
                    ...
                    esc_html__( 'Drag and drop or click on the form to add file.', 'readabler' ),
                    $field[ 'description' ] ?? '',
                    isset( $field[ 'file_types' ] )
                        ? esc_html__( 'Allowed file types:', 'readabler' ) . ' <b>' . esc_attr( $file_types ) . '</b>' : ''
                );
    ...
    public function render_sides( $key, $tab_slug, $field ) {
        ...
        foreach ( self::$css_sides as $number_key ) {
            ...
            UI::get_instance()->render_input(
                $values[ $number_key ],
	            $field[ 'labels' ][ $number_key ] ?? esc_html__( $number_key, 'readabler' ),
                '',
                [
    ...
    private function get_control_id( $key, $tab_slug, $field ) {
        ...
        $id = 'mdp_readabler_' . $tab_slug . '_settings_' . $key;
        return apply_filters( 'mdp_readabler_settings_control_id', $id, $key, $tab_slug, $field );
    ...
    private function get_control_name( $key, $tab_slug, $field ) {
        ...
        $name = 'mdp_readabler_' . $tab_slug . '_settings[' . $key . ']';
        return apply_filters( 'mdp_readabler_settings_control_name', $name, $key, $tab_slug, $field );
	...
	private function get_sides_values( $options, $slug, array $default = array() ): array {
		...
		return apply_filters( 'readabler_settings_side_values', $values, $slug );
    ...
    private function get_sides_unit( $options, $slug, array $default = array() ) {
        ...
        return apply_filters( 'readabler_settings_sides_unit', $unit, $slug );
    ...
    private function get_sides_linked( $options, $slug, array $default = array() ) {
        ...
        return apply_filters( 'readabler_settings_sides_linked', $linked, $slug );
	...
	public function render_unit_slider( $key, $tab_slug, $field ) {
		...
		UI::get_instance()->render_select(
			...
			esc_html__( 'Unit', 'readabler' ),
			'',
			[
				'id' => 'mdp_readabler_' . $tab_slug . '_settings_' . $key . '_unit',
				'name' => 'mdp_readabler_' . $tab_slug . '_settings[' . $key . '_unit' . ']',
			]
		);
		echo '</div>';
