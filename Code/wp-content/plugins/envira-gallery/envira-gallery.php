...
if ( version_compare( PHP_VERSION, '5.6.0', '<' ) ) {
	...
	wp_die( sprintf( esc_html__( 'Envira Gallery requires PHP 5.6 or higher. You\'re still on PHP %s. Please contact your web host for more information on how to upgrade your PHP version.', 'Envira Gallery' ), esc_html( phpversion() ) ) );
...
if ( ! class_exists( 'Envira_Gallery' ) ) :
	...
	final class Envira_Gallery {
		...
		/**
		 * The Compression_Container class.
		 *
		 * @var Compression_Container
		 */
		private $compression;
		...
		public function check_installation() {
			...
			if ( ! empty( $_REQUEST['close_supersize_notice'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			if ( ! empty( $_REQUEST['close_standalone_notice'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		public function beta_notice() {
			...
			echo ( new Notices() )->display_inline_notice( 'beta_noticexxx', false, 'Apply to our exclusive <a href="https://enviragallery.com/account/become-a-beta-tester" />Beta Test Program</a> to see new updates first!', 'updated', '', '', true, 0 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		/**
		 ...
		 * @return array|bool Array of gallery data or false if none found.
		 */
		public function _get_gallery( $id ) { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		 ...
		 * @return array|bool  Array of gallery data or false if none found.
		 */
		public function _get_gallery_by_slug( $slug ) { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		 ...
		 */
		public function _get_galleries( $skip_empty = true, $search_terms = '' ) { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		 ...
		 * @return string
		 */
		public static function mb_substr( $text, $offset, $length ) {
		 ...
		 * @static
		 *
		 * @param mixed $php_class The class.
		 *
		 * @return void
		 */
		public static function autoload( $php_class ) {
			...
			if ( 0 !== strncmp( $prefix, $php_class, $length ) ) {
				return;
			}

			// Prepare classes to be autoloaded.
			$relative_class = self::mb_substr( $php_class, 0, strlen( $php_class ) );
		...
		public static function activate( $network_wide ) {
			...
			if ( version_compare( $wp_version, '4.4.0', '<' ) && ! defined( 'ENVIRA_FORCE_ACTIVATION' ) ) {
				...
				/* translators: %s: admin url */
				wp_die( sprintf( wp_kses_post( 'Sorry, but your version of WordPress does not meet Envira Gallery\'s required version of <strong>4.4.0</strong> to run properly. The plugin has been deactivated. <a href="%s">Click here to return to the Dashboard</a>.', 'envira-gallery' ), esc_url( get_admin_url() ) ) );
			}
