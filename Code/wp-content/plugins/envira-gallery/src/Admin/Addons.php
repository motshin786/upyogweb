...
class Addons {
	...
	public function addons_page() {
		...
		<div id="addon-heading" class="subheading clearfix">
			...
			<form id="add-on-search">
				...
				<input id="add-on-searchbox" name="envira-addon-search" value="" placeholder="<?php echo esc_attr( 'Search ' . ( apply_filters( 'envira_whitelabel', false ) ? '' : 'Envira ' ) . 'Addons' ); ?>" />
	...
	public function addons_content() {
		...
		if ( ! $addons && isset( $_SERVER['REQUEST_URI'] ) ) {
			?>
			<form id="envira-addons-refresh-addons-form" method="post">
				...
				<p>
					<a href="<?php echo esc_url( sanitize_url( wp_unslash( $_SERVER['REQUEST_URI'] ) ) ); ?>" class="button button-primary"><?php esc_html_e( 'Refresh Addons', 'envira-gallery' ); ?></a>                   </p>
			</form>
			<?php
			return;
		}
			...
			if ( count( $addons['unlicensed'] ) > 0 ) {
				...
				<div class="envira-addons-area unlicensed" class="envira-clear">
					...
					<p>
						...
						printf(
							wp_kses_post(
								// translators: %1$s is the upgrade URL, %2$s is the whitelabel name.
								__(
									'<strong>Want even more addons?</strong> <a href="%1$s">Upgrade your %2$s account</a> and unlock the following addons.',
									'envira-gallery'
								)
							),
							esc_url_raw( $upgrade_url ),
							esc_html( apply_filters( 'envira_whitelabel', false ) ? '' : 'Envira Gallery ' )
						);
						?>
					</p>
	...
	public function get_addons() {
		...
		foreach ( (array) $addons as $i => $addon ) {
			...
			if ( (
				...
				( 'envira-social' === $addon->slug && 'plus' !== $type && 'pro' !== $type && 'gold' !== $type && 'ultimate' !== $type && 'lifetime' !== $type && 'platinum' !== $type )
	...
	public function get_addons_data( $key ) {
		...
		$license = new License();
		$version = empty( $key ) ? 'get-addons-data-v15' : 'get-addons-data-v16';

		$addons = $license->perform_remote_request( $version, [ 'tgm-updater-key' => $key ] );
	...
	public function is_refreshing_addons() {
		// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified on refresh_addons_action().
	...
	public function refresh_addons_action() {
		return isset( $_POST['envira-gallery-refresh-addons-submit'], $_POST['envira-gallery-refresh-addons'] )
				&& wp_verify_nonce( sanitize_key( $_POST['envira-gallery-refresh-addons'] ), 'envira-gallery-refresh-addons' );
	}
