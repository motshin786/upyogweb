...
class Envira_List_Table {
	 ...
	 * @var array
	 */
	protected $compat_methods = [ 'set_pagination_args', 'get_views', 'get_bulk_actions', 'bulk_actions', 'row_actions', 'months_dropdown', 'view_switcher', 'comments_bubble', 'get_items_per_page', 'pagination', 'get_sortable_columns', 'get_column_info', 'get_table_classes', 'display_tablenav', 'extra_tablenav', 'single_row_columns' ];
	...
	public function __construct( $args = [] ) {
		...
		if ( $args['ajax'] ) {
			add_action( 'admin_footer', [ $this, 'js_vars' ] );
		}
	...
	public function ajax_user_can() {
		die( 'function Envira_List_Table::ajax_user_can() must be over-ridden in a sub-class.' );
	}

	/**
	 * Prepares the list of items for displaying.
	 *
	 * @uses Envira_List_Table::set_pagination_args()
	 *
	 * @since 3.1.0
	 * @abstract
	 */
	public function prepare_items() {
		die( 'function Envira_List_Table::prepare_items() must be over-ridden in a sub-class.' );
	}
	...
	public function search_box( $text, $input_id ) {
		// phpcs:disable WordPress.Security.NonceVerification -- Nonce verification already handled.
...
<p class="search-box">
	...
	protected function bulk_actions( $which = '' ) {
		if ( is_null( $this->_actions ) ) {
			 ...
			 */
			// phpcs:disable WordPress.NamingConventions.ValidHookName.UseUnderscores -- Backward compatibility.
		...
		echo '<label for="bulk-action-selector-' . esc_attr( $which ) . '" class="screen-reader-text">' . esc_html__( 'Select bulk action', 'envira-gallery' ) . '</label>';
		echo '<select name="action' . absint( $two ) . '" id="bulk-action-selector-' . esc_attr( $which ) . "\">\n";
		echo '<option value="-1">' . esc_html__( 'Bulk Actions', 'envira-gallery' ) . "</option>\n";
	...
	protected function months_dropdown( $post_type ) {
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- $wpdb->prepare() already used.
		$months = $wpdb->get_results(
			// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- $wpdb->prepare() already used.
			$wpdb->prepare(
				"
	 ...
	 * @since 3.1.0
	 *
	 * @param string $current_mode Current view mode.
	 */
	protected function view_switcher( $current_mode ) {
		...
		foreach ( $this->modes as $mode => $title ) {
			...
			wp_kses_post(
				printf(
					// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in printf().
	...
	protected function comments_bubble( $post_id, $pending_comments ) {
		...
		if ( $pending_comments ) {
			...
			printf(
				...
				esc_html( $pending_comments_number ),
				$approved_comments ? esc_html__( 'No pending comments', 'envira-gallery' ) : esc_html__( 'No comments', 'envira-gallery' )
			);
		}
	}
	 ...
	 * @param int    $default_value Default amount of items.
	 *
	 * @return int
	 */
	 ...
	 * @since 3.1.0
	 *
	 * @param string $which Location of the pagination (top or bottom).
	 */
	protected function pagination( $which ) {
		...
		$current_url = set_url_scheme( 'http://' . sanitize_text_field( wp_unslash( $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] ) ) );
		...
		if ( $disable_prev ) {
			...
			$page_links[] = sprintf(
				...
				esc_url( add_query_arg( 'paged', max( 1, $current - 1 ), $current_url ) ) . '#!envira-tab-logs',
				__( 'Previous page' ),
				'&lsaquo;'
			);
		}
		...
		if ( $disable_next ) {
			...
			$page_links[] = sprintf(
				...
				esc_url( add_query_arg( 'paged', min( $total_pages, $current + 1 ), $current_url ) ) . '#!envira-tab-logs',
				__( 'Next page' ),
				'&rsaquo;'
			);
		}
	...
	public function get_columns() {
		die( 'function Envira_List_Table::get_columns() must be over-ridden in a sub-class.' );
	}
	...
	/**
	 * Public wrapper for Envira_List_Table::get_default_primary_column_name().
	 *
	 * @since 4.4.0
	 *
	...
	public function print_column_headers( $with_id = true ) {
		...
		$current_url     = ( ! empty( $_SERVER['HTTP_HOST'] && ! empty( $_SERVER['REQUEST_URI'] ) ) ) ? set_url_scheme( 'http://' . sanitize_text_field( wp_unslash( $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] ) ) ) : '';
...
<table class="wp-list-table <?php echo esc_attr( implode( ' ', $this->get_table_classes() ) ); ?>">
	...
	/**
	 * Get a list of CSS classes for the Envira_List_Table table tag.
	 *
	 * @since 3.1.0
	 *
