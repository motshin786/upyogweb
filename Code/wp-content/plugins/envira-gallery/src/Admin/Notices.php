...
class Notices {
	...
	public function is_dismissed( $notice, $check_transient_only = false ) {
		...
		} elseif ( ! $check_transient_only && ! isset( $this->notices[ $notice ] ) ) {
	...
	public function display_inline_notice( $notice, $title, $message, $type = 'success', $button_text = '', $button_url = '', $is_dismissible = true, $seconds = false, $updated = true ) {
		...
		?>
		<div class="<?php echo $updated ? 'updated ' : ''; ?>envira-notice <?php echo sanitize_html_class( $type ); ?> <?php echo sanitize_html_class( ( $is_dismissible ? ' is-dismissible' : '' ) ); ?>" data-seconds="<?php echo esc_attr( $seconds ); ?>" data-notice="<?php echo esc_attr( $notice ); ?>">
			<?php
			// Title.
			if ( ! empty( $title ) ) {
				?>
				<p class="envira-intro"><?php echo wp_kses_post( $title ); ?></p>
				<?php
			}
