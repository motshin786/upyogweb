...
class Metaboxes {
	...
	public function notice_warnings() {
		...
		if ( isset( $_GET['envira_slug_exists'] ) ) {
		  	// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Not needed.
			$duplicate_post_id = intval( $_GET['envira_slug_exists'] );
			if ( $duplicate_post_id ) {
				...
				// Add a notice for the user that this changed, but saving anyway.
				$message = __( '<strong>Envira Gallery</strong>: There was already a post on your site with the same slug. Envira generated a unique slug for this gallery.</a>', 'envira-gallery' );
			}
	...
	public function meta_box_css() {
		...
		?>
		<style>.misc-pub-section:not(.misc-pub-post-status) { display: none; }</style>
		<?php
		...
		<div id="envira-empty-gallery" class="<?php echo esc_attr( $hide ); ?>">
			<div>
				...
				<?php else : ?>
					...
					<p class="envira-help-text"><?php esc_html_e( 'Need some help?', 'envira-gallery' ); ?> <a href="https://enviragallery.com/docs/creating-first-envira-gallery/" target="_blank"><?php esc_html_e( 'Learn how to add media and create a Gallery', 'envira-gallery' ); ?></a></p>
				<?php endif; ?>
				</div>
		</div>
		...
		<p class="envira-intro">
			...
			<small>
				...
				<?php else : ?>
					...
					<a href="https://enviragallery.com/docs/creating-first-envira-gallery/" class="envira-doc" target="_blank">
				...
				<ul class="envira-right-options">
					...
					<li class="envira-select sorting-options">
						<select id="envira-config-image-sort" name="_envira_gallery[random]" class="envira-chosen" data-envira-chosen-options='{ "disable_search":"true", "width": "100%" }'>
							<?php foreach ( envira_get_sorting_options() as $data ) : ?>
					...
					<li class="envira-select sorting-directions">
						<select id="envira-config-image-sort-dir" name="_envira_gallery[sorting_direction]" class="envira-chosen" data-envira-chosen-options='{ "disable_search":"true", "width": "100%" }'>
							...
							<?php foreach ( envira_get_sorting_directions() as $data ) { ?>
	...
	public function config_tab( $post ) {
		...
		if ( isset( $this->gallery_data['id'] ) && isset( $this->gallery_data['config'] ) ) {
				...
				<tr id="envira-config-layouts-box">
						...
						<td>
							<select id="envira-config-layouts" class="image-picker show-labels show-html" name="_envira_gallery[layout]">
								<?php foreach ( envira_get_layouts() as $value => $data ) : ?>
					...
					<tr id="envira-config-columns-box">
						...
						<td>
							<select id="envira-config-columns" name="_envira_gallery[columns]">
								<?php foreach ( envira_get_columns() as $data ) : ?>
						...
						<tr id="envira-config-gallery-justified-last-row">
							...
							<td>
								<select id="envira-config-gallery-last-row" name="_envira_gallery[justified_last_row]">
									<?php foreach ( envira_get_justified_last_row() as $data ) : ?>
					...
					<tbody>
						...
						<tr id="envira-config-display-description-box">
								...
								<td>
									<select id="envira-config-display-description" name="_envira_gallery[description_position]">
										<?php
										foreach ( envira_get_display_description_options() as $data ) {
											?>
						...
						<tr id="envira-config-additional-copy-box-automatic">
							...
							<td>
								<select id="envira-config-gallery-automatic-title-caption" name="_envira_gallery[gallery_automatic_title_caption]">
									<?php foreach ( envira_get_additional_copy_options() as $option_value => $option_name ) : ?>
						...
						<td>
							<select id="envira-config-image-size" name="_envira_gallery[image_size]">
								<?php
								foreach ( envira_get_image_sizes() as $data ) {
									?>
						...
						<td>
							<select id="envira-config-crop-position" name="_envira_gallery[crop_position]">
								<?php
								foreach ( envira_crop_position() as $data ) {
									?>
						...
						<tr id="envira-config-image-sizes-random-box">
							...
							<td>
								...
								foreach ( envira_get_image_sizes( true ) as $data ) {
									?>
			...
			<div id="envira-config-standard-settings-box">
				...
				<table class="form-table">
					...
					<tbody>
						...
						<tr id="envira-config-gallery-theme-box">
							...
							<td>
								<select id="envira-config-gallery-theme" name="_envira_gallery[gallery_theme]">
									<?php foreach ( envira_get_gallery_themes() as $data ) : ?>
						...
						<tr id="envira-config-show-more-text-box">
							<th scope="row">
								<label for="envira-config-show-more-text"><?php esc_html_e( 'Show More Text', 'envira-gallery' ); ?></label>
							</th>
							<td>
								<input id="envira-config-show-more-text" type="text" name="_envira_gallery[show_more_text]" value="<?php echo esc_html( envira_get_config( 'show_more_text', $gallery_data, envira_get_config_default( 'show_more_text' ) ) ); ?>" />
								<p class="description"><?php esc_html_e( 'Show more text button.', 'envira-gallery' ); ?></p>
							</td>
						</tr>
			...
			<div id="envira-config-sorting">
				...
				<tr id="envira-config-sorting-box">
					...
					<td>
						...
						<?php
						foreach ( envira_get_sorting_options( false ) as $data ) {
							?>
								<option value="<?php echo esc_html( $data['value'] ); ?>"<?php selected( $data['value'], envira_get_config( 'random', $gallery_data ) ); ?>><?php echo esc_html( $data['name'] ); ?></option>
									<?php
						}
				...
				<tr id="envira-config-sorting-direction-box">
					...
					<td>
						<select id="envira-config-sorting-direction" name="_envira_gallery[sorting_direction]">
							<?php
							foreach ( envira_get_sorting_directions() as $data ) {
								?>
								<option value="<?php echo esc_html( $data['value'] ); ?>"<?php selected( $data['value'], envira_get_config( 'sorting_direction', $gallery_data ) ); ?>><?php echo esc_html( $data['name'] ); ?></option>
								<?php
							}
			...
			<div id="envira-lightbox-settings">
				<table class="form-table">
					<tbody>
						...
						<tr id="envira-config-lightbox-theme-box">
							...
							<td>
								<select id="envira-config-lightbox-theme" name="_envira_gallery[lightbox_theme]">
									<?php foreach ( envira_get_lightbox_themes() as $data ) : ?>
						...
						<tr id="envira-config-lightbox-image-size-box">
							...
							<td>
								<select id="envira-config-lightbox-image-size" name="_envira_gallery[lightbox_image_size]">
									<?php foreach ( envira_get_image_sizes() as $data ) : ?>
						...
						<tr id="envira-config-lightbox-title-display-box">
							...
							<td>
								<select id="envira-config-lightbox-title-display" name="_envira_gallery[title_display]">
									<?php foreach ( envira_get_title_displays() as $data ) : ?>
						...
						<tr id="envira-config-lightbox-arrows-position-box">
								...
								<td>
									<select id="envira-config-lightbox-arrows-position" name="_envira_gallery[arrows_position]">
										<?php foreach ( envira_get_arrows_positions() as $data ) : ?>
						...
						<tr id="envira-config-lightbox-toolbar-position-box">
								...
								<td>
									<select id="envira-config-lightbox-toolbar-position" name="_envira_gallery[toolbar_position]">
										<?php foreach ( envira_get_toolbar_positions() as $data ) : ?>
						...
						<tr id="envira-config-lightbox-open-close-effect-box">
								...
								<td>
									<select id="envira-config-lightbox-open-close-effect" name="_envira_gallery[lightbox_open_close_effect]">
										...
										// Standard Effects.
										foreach ( envira_get_envirabox_open_effects() as $data ) {
											?>
						...
						<tr id="envira-config-lightbox-effect-box">
								...
								<td>
									<select id="envira-config-lightbox-effect" name="_envira_gallery[effect]">
										...
										// Standard Effects.
										foreach ( envira_get_transition_effects() as $data ) {
											?>
											<option value="<?php echo esc_html( $data['value'] ); ?>"<?php selected( $data['value'], $effect ); ?>><?php echo esc_html( $data['name'] ); ?></option>
											<?php
										}
						...
						<tr id="envira-config-lightbox_html_caption">
							<th scope="row">
								<label for="envira-config-lightbox_html_caption"><?php esc_html_e( 'Enable Unlimited HTML on caption?', 'envira-gallery' ); ?></label>
							</th>
								<td>
									<input id="envira-config-lightbox_html_caption" type="checkbox" name="_envira_gallery[lightbox_html_caption]" value="<?php echo esc_html( envira_get_config( 'lightbox_html_caption', $gallery_data ) ); ?>" <?php checked( envira_get_config( 'lightbox_html_caption', $gallery_data ), 1 ); ?> />
									<span class="description"><?php esc_html_e( 'Allows unlimited html on lightbox caption', 'envira-gallery' ); ?></span>
								</td>
							...
							<tr id="envira-config-thumbnails-position-box">
								...
								<td>
									<select id="envira-config-thumbnails-position" name="_envira_gallery[thumbnails_position]">
										<?php foreach ( envira_get_thumbnail_positions() as $data ) : ?>
			...
			<table class="form-table">
				<tbody>
					...
					<tr id="envira-config-title-caption-automatic-mobile">
						...
						<tr id="envira-config-gutter-box-mobile">
							<th scope="row">
								<label for="envira-config-gutter-mobile"><?php esc_html_e( 'Column Gutter Width', 'envira-gallery' ); ?></label>
							</th>
							<td>
								<?php
									$desktop_gutter = envira_get_config( 'gutter', $gallery_data, envira_get_config_default( 'gutter' ) );
								?>
								<input id="envira-config-gutter-mobile" type="number" name="_envira_gallery[gutter_mobile]" value="<?php echo esc_html( envira_get_config( 'gutter_mobile', $gallery_data, $desktop_gutter ) ); ?>" /> <span class="envira-unit"><?php esc_html_e( 'px', 'envira-gallery' ); ?></span>
								<p class="description"><?php esc_html_e( 'Sets the space between the columns (defaults to desktop size value).', 'envira-gallery' ); ?></p>
							</td>
						</tr>
				...
				<p class="envira-intro">
					...
					<small>
						...
						<?php else : ?>
							...
							<a href="https://enviragallery.com/docs/standalone/" class="envira-doc" target="_blank">
					...
					<tr id="envira-config-standalone-box">
							...
							<td>
								<?php if ( function_exists( 'wp_is_block_theme' ) && function_exists( 'get_block_templates' ) && wp_is_block_theme() ) { ?>
									...
									<p class="description"><?php esc_html_e( 'You seem to have a block theme active. By default block themes use the single template.', 'envira-gallery' ); ?> <a href="<?php echo esc_url( admin_url( 'site-editor.php?postType=wp_template' ) ); ?>"><?php esc_html_e( 'View your templates here' ); ?></a></p>
									<?php else : ?>
					...
					<tr id="envira-config-copy-images">
						...
						<td>
							...
							<p class="description">
								<?php
								// Translators: %1$s.
								echo wp_kses_post( sprintf( __( 'Copy images to another existing gallery (will not copy images already existing in target gallery). If your published gallery is not showing in this list try <a href="%1$s">clearing transients</a>.', 'envira-gallery' ), esc_url( admin_url( 'edit.php?post_type=envira&page=envira-gallery-tools' ) ) ) );
								?>
							</p>
						</td>
					</tr>
	...
	/**
	 * Display the upsell notices.
	 */
	public function upsell_notice() {
		$license_level = envira_get_license_level();
		$upgrade_link  = ( 'unlicensed' === $license_level ) ? 'https://enviragallery.com/pricing/' : 'https://enviragallery.com/pricing/?license_key=' . envira_get_license_key();

		$upgrading       = esc_html__( 'By upgrading to Envira Pro, you can get access to numerous other features, including:', 'envira-gallery' );
		$albums          = esc_html__( 'Create Albums & Cover Photos For Your Galleries', 'envira-gallery' );
		$add_videos      = esc_html__( 'Add Videos To Your Galleries from Instagram, Youtube, Twitch and more', 'envira-gallery' );
		$gallery_themes  = esc_html__( 'Add More Themes/Layouts to Your Galleries', 'envira-gallery' );
		$build_instagram = esc_html__( 'Build Galleries from Instagram', 'envira-gallery' );
		$download        = esc_html__( 'Give Users the Ability to Download Your Images', 'envira-gallery' );
		$pagination      = esc_html__( 'Speed Up Your Page Loading Times with Pagination', 'envira-gallery' );
		$sell_photos     = esc_html__( 'Sell Your Photos in WordPress with WooCommerce', 'envira-gallery' );
		$theft           = esc_html__( 'Protect Your Images from Online Theft', 'envira-gallery' );
		$lightroom       = esc_html__( 'Upload Your Photos Directly From Adobe Lightroom to WordPress', 'envira-gallery' );
		$support         = esc_html__( 'Dedicated customer support and so much more!', 'envira-gallery' );

		if ( in_array( $license_level, [ 'unlicensed', 'basic' ], true ) ) {
        ...
        <div class="two-column-list">
            <ul>
              <li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/pricing', 'basicadminpagegallery', 'albums' ) . '">' . $albums . '</a></li>
              <li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/pricing', 'basicadminpagegallery', 'addvideos' ) . '">' . $add_videos . '</a></li>
              <li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/pricing', 'basicadminpagegallery', 'gallerythemes' ) . '">' . $gallery_themes . '</a></li>
              <li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/pricing', 'basicadminpagegallery', 'buildinstagram' ) . '">' . $build_instagram . '</a></li>
            </ul>
            <ul>
              <li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/pricing', 'basicadminpagegallery', 'buildinstagram' ) . '">' . $download . '</a></li>
              <li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/pricing', 'basicadminpagegallery', 'pagination' ) . '">' . $pagination . '</a></li>
              <li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/pricing/', 'basicadminpagegallery', 'sellPhotos' ) . '">' . $sell_photos . '</a></li>
               <li>' . $support . '</li>
            </ul>
				</div>',
				'warning',
        ...
        <div class="two-column-list">
          <ul>
            <li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/pricing/', 'plusadminpagegallery', 'albums' ) . '">' . $albums . '</a></li>
            <li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/pricing/', 'plusadminpagegallery', 'addvideos' ) . '">' . $add_videos . '</a></li>
            <li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/pricing/', 'plusadminpagegallery', 'buildinstagram' ) . '">' . $download . '</a></li>
            <li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/pricing/', 'plusadminpagegallery', 'sellPhotos' ) . '">' . $sell_photos . '</a></li>
          </ul>
          <ul>
            <li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/pricing/', 'plusadminpagegallery', 'protection' ) . '">' . $theft . '</a></li>
            <li><a target="_blank" href="' . envira_get_upgrade_link( 'https://enviragallery.com/pricing/', 'plusadminpagegallery', 'lightroom' ) . '">' . $lightroom . '</a></li>
            <li>' . $support . '</li>
          </ul>
				</div>',
				'warning',
	...
	public function save_meta_boxes( $post_id, $post ) {
		...
		if ( ! isset( $_POST['envira-gallery'] ) || ! wp_verify_nonce( sanitize_key( $_POST['envira-gallery'] ), 'envira-gallery' ) || ! isset( $_POST['_envira_gallery'] ) ) {
			return;
		}
		...
		$settings['config']['gutter_mobile']      = isset( $_POST['_envira_gallery']['gutter_mobile'] ) ? absint( $_POST['_envira_gallery']['gutter_mobile'] ) : envira_get_config_default( 'gutter' );
		...
		$settings['config']['show_more_text']     = isset( $_POST['_envira_gallery']['show_more_text'] ) ? sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['show_more_text'] ) ) : '';
		...
		$settings['config']['image_sizes_random']   = isset( $_POST['_envira_gallery']['image_sizes_random'] ) ? array_map( 'sanitize_text_field', wp_unslash( $_POST['_envira_gallery']['image_sizes_random'] ) ) : [];
		...
		$settings['config']['lightbox_html_caption'] = isset( $_POST['_envira_gallery']['lightbox_html_caption'] ) ? 1 : 0;
		...
		if ( class_exists( 'Envira_Albums' ) && function_exists( '_envira_get_albums' ) ) :
			...
			if ( ! empty( $albums ) ) :
				foreach ( $albums as $album ) {
					if ( isset( $_POST['post_title'] ) && isset( $album['id'] ) && ! empty( $album['galleries'][ $post_id ] ) ) {
						$album['galleries'][ $post_id ]['title'] = esc_html( sanitize_text_field( wp_unslash( $_POST['post_title'] ) ) );
		...
		$settings['config']['slug'] = empty( $slug_to_save ) ? false : $slug_to_save;
