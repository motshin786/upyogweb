...
class Import {
	...
	public function import_gallery() {
		...
		$post_id = isset( $_POST['envira_post_id'] ) ? absint( $_POST['envira_post_id'] ) : null; // phpcs:ignore WordPress.Security.NonceVerification.Missing
	...
	public function import_gallery_item( $id, $item, $data, $post_id ) {
		...
		} elseif ( $image[0] !== $item['src'] ) {
			// Stream the image from a remote URL.
			$data = $this->import_remote_image( $item['src'], $data, $item, $post_id, $id );
		} else {
			// The URLs match. We can simply update data and continue.
			$this->update_gallery_checker( $id, $post_id );
		}
	...
	public function has_imported_gallery() {
		...
		return ! empty( $_POST['envira_import'] );  // phpcs:ignore WordPress.Security.NonceVerification.Missing
	}
	...
	public function verify_imported_gallery() {
		...
		return isset( $_POST['envira-gallery-import'] ) && wp_verify_nonce( sanitize_key( $_POST['envira-gallery-import'] ), 'envira-gallery-import' );
	}
	...
	public function post_can_handle_gallery() {
		...
		return isset( $_POST['envira_post_id'] ) && ! wp_is_post_revision( sanitize_text_field( wp_unslash( $_POST['envira_post_id'] ) ) );  // phpcs:ignore WordPress.Security.NonceVerification.Missing
	}
	...
	public function has_imported_gallery_files() {
		...
		return ! empty( $_FILES['envira_import_gallery']['name'] ) || ! empty( $_FILES['envira_import_gallery']['tmp_name'] ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
	}
	...
	public function has_correct_filename() {
		...
		return isset( $_FILES['envira_import_gallery']['name'] ) ? preg_match( '#^envira-gallery#i', sanitize_text_field( wp_unslash( $_FILES['envira_import_gallery']['name'] ) ) ) : false;  // phpcs:ignore WordPress.Security.NonceVerification.Missing
	}
	...
	public function has_json_extension() {
		...
		$file_array = isset( $_FILES['envira_import_gallery']['name'] ) ? explode( '.', sanitize_text_field( wp_unslash( $_FILES['envira_import_gallery']['name'] ) ) ) : null; // phpcs:ignore WordPress.Security.NonceVerification.Missing
	...
	public function get_file_contents() {
		...
		$file = isset( $_FILES['envira_import_gallery']['tmp_name'] ) ? sanitize_text_field( wp_unslash( $_FILES['envira_import_gallery']['tmp_name'] ) ) : false; // phpcs:ignore WordPress.Security.NonceVerification.Missing
	...
	public function notices() {
		...
		if ( empty( $this->errors ) && isset( $_GET['envira-gallery-imported'] ) && sanitize_text_field( wp_unslash( $_GET['envira-gallery-imported'] ) ) ) : // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			?>
