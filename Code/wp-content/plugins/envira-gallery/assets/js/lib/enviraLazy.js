...
var enviraLazy = (function() {
	...
	var updateElement = function(container, element) {
			...
			if (options.length > 0) {
				...
				for (var j = 0; j < optionsCount; j++) {
					...
					if (spaceIndex === -1) {
						...
						var optionWidth = parseInt(
							option.substr(
								spaceIndex + 1,
								option.length - spaceIndex - 2,
							),
							10,
						);
					}
				...
				temp.sort(function(a, b) {
					...
					if (a[1] === b[1]) {
						if (
							b[0].indexOf('.webp', b[0].length - 5) !== -1
						) {
							return 1;
						}
						if (
							a[0].indexOf('.webp', a[0].length - 5) !== -1
						) {
							return -1;
						}
					}
		...
		if (bestSelectedOption === null) {
			bestSelectedOption = [
				element.getAttribute('data-envira-src'),
				999999,
			];
		}
		...
		if (container.lastSetOption[1] < bestSelectedOption[1]) {
			...
			image.addEventListener(
				...
				function() {
					...
					if (fireEvent) {
						var handler = container.getAttribute(
							'data-onlazyload',
						);
						if (handler !== null) {
			...
			image.onload = function() {
				if (
					container.getAttribute('class') == 'envira-lazy' &&
					$(container).not('img')
				) {
					...
					var gallery_id = jQuery(the_image).data(
						'envira-gallery-id',
					);
					...
					var gallery_id = jQuery(container).data(
						'envira-gallery-id',
					);
					var item_id = jQuery(container).data('envira-item-id');
					var naturalWidth = this.naturalWidth;
	...
	var run = function(galleryClass) {
		...
		if (galleryClass) {
			...
			if (
				'undefined' === typeof envira_gallery.ll_initial
				|| 'undefined' === typeof envira_gallery.ll_delay
				|| envira_gallery.ll_initial === false
			) {
			...
			setTimeout(function() {
				if (
					jQuery(
						galleryClass + ' div.envira-lazy > img',
					).exists()
				) {
					// alert('1');
					update(
					...
					jQuery(galleryClass + ' img.envira-lazy').exists()
				) {
					// alert('2');
					update(
						document.querySelectorAll(
							galleryClass + ' img.envira-lazy',
						),
						true,
					);
				}
	...
	if (
		...
		var image = new Image();
		image.src =
			'data:image/webp;base64,UklGRiQAAABXRUJQVlA4IBgAAAAwAQCdASoCAAEADMDOJaQAA3AA/uuuAAA=';
			...
			if (hasIntersectionObserverSupport) {
				var updateIntersectionObservers = function() {
					...
					for (var i = 0; i < elementsCount; i++) {
						var element = elements[i];
						if (
							typeof element.responsivelyLazyObserverAttached ===
							'undefined'
						) {
				...
				var runIfHasChange = function() {
					...
					requestAnimationFrameFunction.call(
						null,
						runIfHasChange,
					);
				};
				...
				var updateParentNodesScrollListeners = function() {
					...
					for (var i = 0; i < elementsCount; i++) {
						...
						while (
							...
							if (
								typeof parentNode.responsivelyLazyScrollAttached ===
								'undefined'
							) {
